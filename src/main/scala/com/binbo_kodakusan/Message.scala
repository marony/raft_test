...
case class GetLogReply(val serverId: ServerId, val role: Role, val votedFor: ServerId, val commitIndex: Int, val lastApplied: Int, val entries: Seq[(Term, Command)]) extends Reply
