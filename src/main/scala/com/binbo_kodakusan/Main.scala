...
// クライアントから送られるコマンド(具体的にはなんでもいい)
case class Command(val something: String)
...
class RaftActor(mySettingIndex: Int, serverSettings: Array[ServerSetting]) extends Actor {
  ...
  var role: Role = Initializing
  val serverPersistentState = ServerPersistentState(Term(0), ServerNone, Vector())
      ...
      if (role == Leader) {
        ...
        if (role != Leader) {
          serverSettings.find(_.serverId == serverPersistentState.votedFor) match {
            case Some(s) =>
              val path = s"/user/raftActor-${s.serverId.id}"
              log(s"-> $path")
  ...
  def log(log: String) = {
    println(s"${Thread.currentThread().getName}: ${mySetting.serverId}($role)(votedFor=${serverPersistentState.votedFor})(commitIndex=${serverState.commitIndex}, lastApplied=${serverState.lastApplied}): $log")
  }
}
...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    val settings = Array(
      ServerSetting(SpecificId(1), null),
      ServerSetting(SpecificId(2), null),
      ServerSetting(SpecificId(3), null)
    )

    var as: Vector[ActorRef] = Vector()
    for (i <- 0 until settings.length) {
      val path = s"raftActor-${settings(i).serverId.id}"
      val actor = system.actorOf(Props(new RaftActor(i, settings)).withDispatcher(dispatcher), name = path)
